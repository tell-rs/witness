...
- Polling-based tailing — 250ms base poll with adaptive backoff to 2s when idle, 50ms fast catchup under load.
...
- `witness setup` — fetches config from a Tell server or generates sensible defaults. Validates API key format before any network call.
...
- Dry-run mode — `--dry-run` shows what would be collected without sending, runs two ticks to display delta metrics.
